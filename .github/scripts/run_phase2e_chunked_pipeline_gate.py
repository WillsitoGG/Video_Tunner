...
from video_tunner.transcription import (
    CHUNKED_TRANSCRIPTION_HOP_SECONDS,
...
EXPECTED_CHUNK_COUNT_FOR_RENDER_FIXTURES = 7
...
def _evaluate_case(
    ...
    *,
    ...
    strategy_ok = (
        ...
        and strategy.get("name") == CHUNKED_TRANSCRIPTION_STRATEGY
        and abs(float(strategy.get("chunk_window_seconds")) - CHUNKED_TRANSCRIPTION_WINDOW_SECONDS) <= 1e-9
        and abs(float(strategy.get("chunk_hop_seconds")) - CHUNKED_TRANSCRIPTION_HOP_SECONDS) <= 1e-9
        and int(strategy.get("chunk_count") or 0) == EXPECTED_CHUNK_COUNT_FOR_RENDER_FIXTURES
    )
    ...
    print(
        f"PHASE2E_CHUNKED_PIPELINE_CASE={case_id} "
...
def main() -> int:
    ...
    for case in spec.get("cases") or []:
        try:
        ...
        except Exception as exc:  # Keep the manifest even when one real case fails.
            ...
            print(f"PHASE2E_CHUNKED_PIPELINE_HARD_FAILURE={message}")

    pass_count = sum(1 for item in results if item.get("pass"))
    source_ids = {str(item.get("audio_source_id")) for item in results if item.get("audio_source_id")}
    gate_pass = (
        len(results) == 3
        and pass_count == 3
    ...
    print(f"PHASE2E_CHUNKED_PIPELINE_PASS_COUNT={pass_count}/3")
    print(f"PHASE2E_CHUNKED_PIPELINE_DISTINCT_SOURCES={len(source_ids)}")
    print(f"PHASE2E_CHUNKED_PIPELINE_GATE={manifest['gate']}")
    print(f"PHASE2E_CHUNKED_PIPELINE_MANIFEST={manifest_path}")
