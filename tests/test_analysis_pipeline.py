...
class AnalysisPipelineTests(unittest.TestCase):
    ...
    def test_pipeline_uses_the_same_verified_master_for_analysis_layers(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(
                len(report["promotion_assessments"]),
                len(report["eligibility_assessments"]),
            )
            ...
            self.assertEqual(report["summary"]["promotion_assessments"]["safe_for_cut"], 0)
            self.assertEqual(report["summary"]["promotion_assessments"]["executable"], 0)
            self.assertEqual(report["summary"]["promotion_assessments"]["auto_apply"], 0)
            ...
            for assessment in report["eligibility_assessments"]:
                ...
                self.assertFalse(assessment["executable"])
                self.assertFalse(assessment["auto_apply"])
            for assessment in report["promotion_assessments"]:
                self.assertFalse(assessment["approved"])
                self.assertIsNone(assessment["edit"])
                self.assertFalse(assessment["safe_for_cut"])
                self.assertFalse(assessment["executable"])
                self.assertFalse(assessment["auto_apply"])
